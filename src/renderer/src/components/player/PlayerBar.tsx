...
export function PlayerBar(): JSX.Element {
  ...
  return (
    <div className="h-24 bg-bg-surface border-t border-border-default flex items-center px-4 gap-4 relative transition-colors duration-200">
      ...
      <div className="flex-1 flex flex-col items-center gap-1">
        ...
        <div className="flex items-center gap-4">
          <button
            ...
            className={`text-sm transition ${shuffle ? 'text-accent' : 'text-text-muted hover:text-text-secondary'}`}
            title="Shuffle"
          >
            ⇌
          </button>
          ...
          <button
            onClick={cycleRepeat}
            className={`text-sm transition ${repeat !== 'off' ? 'text-accent' : 'text-text-muted hover:text-text-secondary'}`}
            title={`Repeat: ${repeat}`}
          >
            {repeatIcons[repeat]}
          </button>
      ...
      <div className="flex items-center gap-3 w-60 justify-end relative">
        <VolumeControl />
        <button
